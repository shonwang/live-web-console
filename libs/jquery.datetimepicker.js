	...
	$.fn.datetimepicker = function( opt ) {
		var KEY0 = 48,
			...
			createDateTimePicker = function( input ) {
				...
				var setPos = function() {
					var offset, top, left;
					if (options.customPos.get(0)){
						offset = options.customPos.offset();
						top    = offset.top + options.customPos[0].offsetHeight - 1;
						left   = offset.left;
						if (options.offleft) left = left + datetimepicker[0].offsetWidth;
						if( top + datetimepicker[0].offsetHeight > $(window).height() + $(window).scrollTop())
							top = offset.top - datetimepicker[0].offsetHeight+1;
						if( left + datetimepicker[0].offsetWidth > $(window).width()){
							if (options.offleft) {
								top = top + datetimepicker[0].offsetHeight;
								left= offset.left;
								if( left + datetimepicker[0].offsetWidth > $(window).width())
									left = offset.left - datetimepicker[0].offsetWidth + options.customPos[0].offsetWidth;
							} else {
								left = offset.left - datetimepicker[0].offsetWidth + options.customPos[0].offsetWidth;
							}
						}
					} else {
						offset = datetimepicker.data('input').offset();
